...
def write_mbfxml(output_mbf, contents, options):
    data = contents["contours"] if "contours" in contents else contents["trees"]
    # data = {
    #     'contours': {
    ...
    for point in data['points'][0]:
        ET.SubElement(contour, "point", x=str(point[0]), y=str(point[1]), z=str(point[2]), d=str(point[3]), sid="S1072")
